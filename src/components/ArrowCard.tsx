...
type Props = {
  entry: CollectionEntry<"blog"> | CollectionEntry<"projekte">
  pill?: boolean
}
    ...
    return (
      ...
      <div className="w-full group-hover:text-black blend">
        <div className="flex flex-wrap items-center gap-2">
          {pill &&
            <div className="text-sm capitalize px-2 py-0.5 rounded-full border border-black/15">
        ...
        <div className="font-semibold mt-3 text-black">
          {entry.data.title}
        </div>
        ...
        <ul className="flex flex-wrap mt-2 gap-1">
          {entry.data.tags.map((tag:string) => ( // this line has an error; Parameter 'tag' implicitly has an 'any' type.ts(7006)
            <li className="text-xs uppercase py-0.5 px-1 rounded bg-black/5 text-black/75">
              {tag}
            </li>
          ))}
        </ul>
      </div>
      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" className="stroke-current group-hover:stroke-black">
